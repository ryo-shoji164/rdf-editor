      ...
      if (quad) {
        ...
        if (prefixMap) {
          prefixes = Object.fromEntries(
            Object.entries(prefixMap as Record<string, N3.NamedNode>).map(
              ([k, v]) => [k, typeof v === 'string' ? v : v.value]
            )
          )
        }
 ...
 */
export function parseNTriples(text: string, format: 'N-Triples' | 'N-Quads' = 'N-Triples'): Promise<ParseResult> {
  return new Promise((resolve) => {
    const store = new N3.Store()
...
export async function parseJsonLd(text: string): Promise<ParseResult> {
  try {
    ...
    const nquads: string = await (jsonld.default as typeof import('jsonld')).toRDF(doc, {
